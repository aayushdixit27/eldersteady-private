...
Before presenting: open `interface/demo.html`, start the phone page with `interface/serve-phone.sh`, and keep a terminal running `perception/live_demo.sh`. Confirm pose mode, camera preview, lean gauge, live counter, and serial access. Keep the fixture backup closed unless the live path fails.
...
“At 60 frames, this morning’s pre-quiet-mode run showed 165.9 MB COMPUTED pixels, 2.55 MB MEASURED video in, and 15.2 KB MEASURED out: 1 to 168, COMPUTED. The 15.2 KB included SSH per-frame diagnostics. Capture 0833 measured quiet mode at 11.1 KB out at frame 150, 1 to 303 COMPUTED.”

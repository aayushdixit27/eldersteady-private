...
docker exec -u aayushdixit $SDK ssh -t -o BatchMode=yes sima@$BOARD \
  "cd / && source ~/pyneat/bin/activate && python3 /home/sima/watch-perception/watch_events.py \
     ...
     2>&1 | grep --line-buffered -E '^(frame=|done|\{)' | sed -u -E 's/^frame=([0-9]+) processed=[0-9]+ detections=([0-9]+) best=([0-9.]+) infer_ms=([0-9.]+)/frame \1   people-in-view=\2   confidence=\3   mla=\4ms   uploaded=0/; s/^frame=([0-9]+) processed=[0-9]+ posture=([^ ]+) lean=([0-9]+)% poses=([0-9]+) valid_lean=([0-9]+) best=([0-9.]+) bent_streak=([0-9]+) infer_ms=([0-9.]+)/frame \1   posture=\2   lean=\3%   poses=\4   confidence=\6   streak=\7   mla=\8ms   uploaded=0/' | tee -a "$LIVE/board.log""
